...
struct PillarExtractorImpl {
    ...
    void findPillars(std::vector<pcl::PointIndices>& cluster_indices,
                     pcl::PointCloud<pcl::PointXYZI>::Ptr& cloud,
                     std::vector<Eigen::Vector3d, Eigen::aligned_allocator<Eigen::Vector3d> >& pillar_candidates);
...
std::vector<Eigen::Vector3d, Eigen::aligned_allocator<Eigen::Vector3d> > PillarExtractor::findPillars(const sensor_msgs::PointCloud2::ConstPtr& input)
{
    return pimpl->findPillars(input);
}

std::vector<Eigen::Vector3d, Eigen::aligned_allocator<Eigen::Vector3d> > PillarExtractorImpl::findPillars(const sensor_msgs::PointCloud2::ConstPtr& input)
{
                 ...
                 std::vector<Eigen::Vector3d, Eigen::aligned_allocator<Eigen::Vector3d> >& pillar_candidates)
{
        ...
        ++it)
    {
        ...
        if(std::isfinite(dist_sqr)) {
            ...
            pillar_candidates.push_back(centre);
        }
    }

//    std::sort(pillar_candidates.begin(), pillar_candidates.end(), std::greater<std::pair<int,Eigen::Vector3d> >());
}
